...
当用户报"款式+张数+地址"，先算总重，再算运费。总重 = 产品净重 + 木箱空箱重量。
...
### 木箱空箱重量
**一票货只打一个木箱**，木箱尺寸以其中最大材料尺寸为准（各边+8cm）。

如同时有黑山岩3.1×1.16和玄武岩3.0×0.6，按最大的黑山岩算一个箱即可，不是两个箱。

木箱重量按最大那张的长度分档，宽板全重，窄板（宽<1m）半重：

| 产品长度 | 宽度≥1m（全重） | 宽度<1m（半重） |
|----------|---------------|----------------|
| ≥3.0m | 60 kg | 30 kg |
| 2.6~2.99m | 50 kg | 25 kg |
| 2.0~2.59m | 40 kg | 20 kg |
| <2.0m | 30 kg | 15 kg |

总重 = 各款产品净重之和 + 一个木箱重量（按最大那张算）

### 计算示例
"黑山岩 5张 + 玄武岩 3张 发合肥"
- 黑山岩: 3.1×1.16m, 10kg/㎡ → 3.1×1.16×10×5 = 179.8kg
- 玄武岩: 3.0×0.6m, 7.5kg/㎡ → 3.0×0.6×7.5×3 = 40.5kg
- 净重合: 179.8+40.5 = 220.3kg
- 最大尺寸为黑山岩3.1×1.16 → 长≥3m宽≥1m → 木箱 60kg
- 总重: 220.3+60 = 280.3kg ≈ 280kg
- 然后计算280kg到合肥的运费

### 按面积算（平方）
如用户报"夯土A 50平方"，直接用面积×重量/㎡：
- 夯土A 6.5kg/㎡ × 50㎡ = 325kg（不含箱）
- 50平方需要多少张: 50÷(2.36×0.58)≈37张，按张数估算木箱
